...
void testVector()
{
    ...
    buffer << t;
    check(buffer, "0 0 0\n", "Construct a new vector with default constructor");

    buffer << a;
    check(buffer, "3 2 5\n", "Construct a new vector with 3 given doubles");

    buffer << c;
    check(buffer, "5 1 4\n", "Construct a new vector with array of 3 doubles");

    Vector3D r(c);
    buffer << r;
    check(buffer, "5 1 4\n", "Construct a new vector with copy constructor");

    // Test Operators

    Vector3D e = b + c;
    buffer << e;
    check(buffer, "12 2 4\n", "Test '+' operator. [" + getString(b) + "] + [" + getString(c) + "]");

    buffer << a - b;
    check(buffer, "-4 1 5\n", "Test '-' operator. [" + getString(a) + "] - [" + getString(b) + "]");
    ...
    c += d;
    buffer << b;
    check(buffer, "2 0 -4\n", "Test '-=' operator. [" + tempB + "] -= [" + tempC + "]");
    buffer << c;
    check(buffer, "20 6 14\n", "Test '+=' operator. [" + tempC + "] += [" + getString(d) + "]");

    buffer << -d;
    check(buffer, "-15 -5 -10\n", "Test 'unary -' operator. -[" + getString(d) + "]");
    ...
    b += 3;
    buffer << a;
    check(buffer, "-2 -3 0\n", "Test '-=' operator. [" + tempA + "] -= 5");
    buffer << b;
    check(buffer, "6 3 -1\n", "Test '+=' operator. [" + tempB + "] += 3");

    Vector3D f = a * 5;
    Vector3D g = b / 2;
    buffer << f;
    check(buffer, "-10 -15 0\n", "Test '*' operator. [" + getString(a) + "] * 5");
    buffer << g;
    check(buffer, "3 1.5 -0.5\n", "Test '/' operator. [" + getString(a) + "] / 5");

    f = 2 * b;
    buffer << f;
    check(buffer, "12 6 -2\n", "Test '*' operator. 2 * [" + getString(b) + "]");
    ...
    g /= 0.5;
    buffer << f;
    check(buffer, "60 30 -10\n", "Test '*=' operator. [" + tempF + "] *= 5");
    buffer << g;
    check(buffer, "6 3 -1\n", "Test '/=' operator. [" + tempG + "] /= 0.5");
    ...
    std::cin.rdbuf(orig);
    buffer << f;
    check(buffer, "5 4 3\n", "Test '>>' operator. Read \"5 4 3\" from stdin into vector");
...
void testMatrix()
{
    ...
    buffer << a;
    check(buffer, "0 0 0\n0 0 0\n0 0 0\n", "Construct a new matrix with default constructor");

    buffer << b;
    check(buffer, "8 0 0\n0 8 0\n0 0 8\n", "Construct a new matrix with a given double (8) in diagonal");

    buffer << c;
    check(buffer, "1 2 3\n4 5 6\n7 8 9\n", "Construct a new matrix with 9 given doubles");

    buffer << d;
    check(buffer, "2 4 6\n8 10 12\n14 16 18\n", "Construct a new matrix with a given [9] array");

    buffer << e;
    check(buffer, "3 6 9\n12 15 18\n21 24 27\n", "Construct a new matrix with a given [3][3] array");

    Matrix3D r(c);
    buffer << r;
    check(buffer, "1 2 3\n4 5 6\n7 8 9\n", "Construct a new matrix with copy constructor");
    ...
    std::cin.rdbuf(orig);
    buffer << m;
    check(buffer, "6 2 3\n4 5 8\n7 9 5\n", "Test '>>' operator. Read \"6 2 3 4 5 8 7 9 5\" from stdin into matrix");

    buffer << c[2];
    check(buffer, "35 40 45\n", "Test '[]' operator. Get [2] line of: \n" + getString(c));

    std::string tempB = getString(b);
    b[0] = {5, 6, 7};
    buffer << b;
    check(buffer, "5 6 7\n0 8 0\n0 0 8\n", "Test '[]' operator. Change [0] line of: \n" + tempB + "\nwith [5, 6, 7]");
    ...
    buffer << b.row(2);
    check(buffer, "-1020 -1260 -1492\n", "Run \"row(2)\" function on: \n" + getString(b));

    buffer << c.column(1);
    check(buffer, "364 820 1276\n", "Run \"column(1)\" function on: \n" + getString(c));
